...
import leftBoxVue from "./leftBox.vue";
...
export default {
  ...
  methods: {
    ...
    houseInfo() {
      this.$axios({
        ...
        if (res.data.code == 1000) {
          ...
          //信息放入vuex
            this.$store.state.commonData.leftBoxHouseInfo = leftBoxHouseInfo;
          /* 一次性渲染所有楼栋 */
          this.house();
        }
        console.log(res);
      ...
      this.$axios({
        ...
        if (res.data.code == 1000) {
          ...
          res.data.data.forEach(item => {
            ...
            //信息放入vuex
            this.$store.state.commonData.leftBoxMonitoringInfo = leftBoxMonitoringInfo;
          });
          /* 一次性渲染所有摄像头 */
          this.monitoring();
        }
    ...
    initMap() {
      ...
      this.map.centerAndZoom(point, 14);//设置中心位置，层级 */
    ...
    community(item) {
      ...
      this.map.centerAndZoom(mapViewObj.center, 18 /* mapViewObj.zoom */);
    ...
    house() {
      //console.log(this.houseListInfo.houseMarker);
      this.map.clearOverlays();
      //添加文本标签
      var houseMarkerArr = [];
      for (var i = 0; i < this.houseListInfo.houseMarker.length; i++) {
        houseMarkerArr.push(
          new BMap.Point(...this.houseListInfo.houseMarker[i])
        );
        var point = new BMap.Point(...this.houseListInfo.houseMarker[i]);
        var opts = {
          position: point, // 指定文本标注所在的地理位置
          offset: new BMap.Size(-30, -20) //设置文本偏移量
        };
        var label = new BMap.Label(this.houseListInfo.buildingNo[i], opts); // 创建文本标注对象
        label.customData = { buildingCode: this.houseListInfo.buildingCode[i] }; //自定义参数id,很关键百度不支持直接传参，闭包策略不适用
        label.addEventListener("click", this.houseList);
        label.setStyle({
          color: "#fff",
          fontSize: "18px",
          height: "48px",

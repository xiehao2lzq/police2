<template>
  <div class="face boxColor">
    ...
    <div class="infoBox">
        <RealTimeList v-for="item in peopleList" :key="item.id+item.timeStamp" :item="item" />
    </div>
  </div>
</template>
...
export default {
  ...
  components: {
   ...
   methods: {
    router(){
      this.$router.push({name:"PersonPerceptionList",query:{tabIndex:1,xqCode:""}})
    }
  }
};
</script>
